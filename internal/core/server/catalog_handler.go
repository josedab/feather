...
// handleGetDashboard handles GET /v1/catalog/dashboard
func (h *CatalogHandler) handleGetDashboard(w http.ResponseWriter, r *http.Request) {
	summary := h.catalog.GetDashboardSummary()
	h.writeJSON(r.Context(), w, http.StatusOK, summary)
}

// handleGetPopular handles GET /v1/catalog/popular
func (h *CatalogHandler) handleGetPopular(w http.ResponseWriter, r *http.Request) {
	limit := 20
	if limitStr := r.URL.Query().Get("limit"); limitStr != "" {
		if parsed, err := strconv.Atoi(limitStr); err == nil && parsed > 0 {
			limit = parsed
	...
	popular := h.catalog.GetPopular(limit)
	h.writeJSON(r.Context(), w, http.StatusOK, map[string]interface{}{
		"features": popular,
		"count":    len(popular),
	})
...
// handleGetRecent handles GET /v1/catalog/recent
func (h *CatalogHandler) handleGetRecent(w http.ResponseWriter, r *http.Request) {
	limit := 20
	if limitStr := r.URL.Query().Get("limit"); limitStr != "" {
		if parsed, err := strconv.Atoi(limitStr); err == nil && parsed > 0 {
			limit = parsed
	...
	recent := h.catalog.GetRecentlyUpdated(limit)
	h.writeJSON(r.Context(), w, http.StatusOK, map[string]interface{}{
		"features": recent,
		"count":    len(recent),
	})
...
// handleRecordView handles POST /v1/catalog/features/{name}/view
func (h *CatalogHandler) handleRecordView(w http.ResponseWriter, r *http.Request) {
	name := r.PathValue("name")
	if name == "" {
		h.writeError(r.Context(), w, http.StatusBadRequest, "feature name required")
		return
