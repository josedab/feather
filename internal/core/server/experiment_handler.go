...
func (h *ExperimentHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /v1/experiments/{id}/auto-decision", h.handleCheckAutoDecision)
	mux.HandleFunc("GET /v1/features/{featureId}/impact", h.handleGetFeatureImpact)
	mux.HandleFunc("GET /v1/experiments/summary", h.handleGetSummary)
}
...
func (h *ExperimentHandler) handleCheckAutoDecision(w http.ResponseWriter, r *http.Request) {
	if h.engine == nil {
		h.writeError(r.Context(), w, http.StatusServiceUnavailable, "experiment engine not configured")
		return
	}
	...
	id := r.PathValue("id")
	config := experiment.DefaultAutoDecisionConfig()
	...
	if r.ContentLength > 0 {
		if err := json.NewDecoder(r.Body).Decode(&req); err == nil {
			...
			if req.MaxPValue != nil {
				config.MaxPValue = *req.MaxPValue
			}
		}
	}

	result, err := h.engine.CheckAutoDecision(id, config)
	if err != nil {
		h.writeError(r.Context(), w, http.StatusNotFound, err.Error())
		return
	}
...
func (h *ExperimentHandler) handleGetFeatureImpact(w http.ResponseWriter, r *http.Request) {
	if h.engine == nil {
		h.writeError(r.Context(), w, http.StatusServiceUnavailable, "experiment engine not configured")
		return
	}
	...
	impact, err := h.engine.GetFeatureImpact(featureID)
	if err != nil {
		h.writeError(r.Context(), w, http.StatusNotFound, err.Error())
		return
	}
...
func (h *ExperimentHandler) handleGetSummary(w http.ResponseWriter, r *http.Request) {
	if h.engine == nil {
		h.writeError(r.Context(), w, http.StatusServiceUnavailable, "experiment engine not configured")
		return
	}

	summary := h.engine.GetExperimentSummary()
