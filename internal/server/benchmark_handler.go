...
import (
	...
	"github.com/feather-store/feather/internal/benchmark"
...
func (h *BenchmarkHandler) handleRunBenchmark(w http.ResponseWriter, r *http.Request) {
	if h.store == nil {
		h.writeError(w, http.StatusServiceUnavailable, "store not configured")
		return
	}
...
func (h *BenchmarkHandler) writeJSON(w http.ResponseWriter, status int, data interface{}) {
	writeJSONResponse(w, status, data)
}

func (h *BenchmarkHandler) writeError(w http.ResponseWriter, status int, message string) {
