...
func TestCatalog_Popularity(t *testing.T) {
	c := NewCatalog()
	_ = c.Register(&FeatureDefinition{Name: "feat1", Owner: "alice"}, "alice")
	_ = c.Register(&FeatureDefinition{Name: "feat2", Owner: "bob"}, "bob")
	...
	if len(popular) != 2 {
		t.Fatalf("expected 2 popular features, got %d", len(popular))
	}
	// feat2 should be first (2 queries * weight 2 = 4 > 2 views * weight 1 = 2)
	if popular[0].FeatureName != "feat2" {
		t.Errorf("expected feat2 first, got %s", popular[0].FeatureName)
	}
}

func TestCatalog_RecentlyUpdated(t *testing.T) {
	c := NewCatalog()
	_ = c.Register(&FeatureDefinition{Name: "old_feat"}, "user")
	_ = c.Register(&FeatureDefinition{Name: "new_feat"}, "user")

	recent := c.GetRecentlyUpdated(1)
	if len(recent) != 1 {
		t.Fatalf("expected 1 recent feature, got %d", len(recent))
	}
}

func TestCatalog_DashboardSummary(t *testing.T) {
	c := NewCatalog()
	_ = c.Register(&FeatureDefinition{Name: "f1", Team: "data", Status: "active"}, "user")
	_ = c.Register(&FeatureDefinition{Name: "f2", Team: "ml", Status: "active"}, "user")
	c.RecordView("f1")

	summary := c.GetDashboardSummary()
	if summary.Stats.TotalFeatures != 2 {
		t.Errorf("expected 2 total features, got %d", summary.Stats.TotalFeatures)
	}
