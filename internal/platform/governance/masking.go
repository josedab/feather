...
func (m *DataMasker) Mask(ctx context.Context, featureName string, value interface{}, userContext *MaskingContext) (interface{}, bool) {
	masked, applied, err := m.MaskWithError(ctx, featureName, value, userContext)
	if err != nil {
		return value, false
	}
...
// MaskWithError applies masking and returns any masking error.
func (m *DataMasker) MaskWithError(ctx context.Context, featureName string, value interface{}, userContext *MaskingContext) (interface{}, bool, error) {
	if !m.config.Enabled {
...
func (m *DataMasker) applyMasking(rule *MaskingRule, value interface{}) (interface{}, error) {
	...
	case MaskingTypeNull:
		return nil, nil
	default:
		return value, fmt.Errorf("%w: %s", ErrInvalidMaskingType, rule.MaskingType)
	}
}
