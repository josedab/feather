...
|-------|------|----------|-------------|
| `storeRef` | string | Yes | Reference to FeatureStore |
| `features` | []FeatureRef | Yes | Features to include |
| `transformations` | []TransformSpec | No | Feature transformations |
| `materialize` | MaterializeSpec | No | Materialization settings |
| `ttl` | duration | No | TTL for materialized data |
...
├─────────────────────────────────────────────────────────────────┤
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐ │
│  │ FeatureStore    │  │ FeatureGroup    │  │ FeatureView     │ │
...
│  ┌───────────┐  ┌───────────┐  ┌───────────┐  ┌───────────┐    │
│  │StatefulSet│  │  Service  │  │ ConfigMap │  │    HPA    │    │
│  └───────────┘  └───────────┘  └───────────┘  └───────────┘    │
└─────────────────────────────────────────────────────────────────┘
```
...
Configure default values for FeatureStore resources:

```yaml
apiVersion: feather.io/v1alpha1
kind: FeatureStoreDefaults
metadata:
...
- [API Reference](./api-reference.md) - HTTP and gRPC APIs
